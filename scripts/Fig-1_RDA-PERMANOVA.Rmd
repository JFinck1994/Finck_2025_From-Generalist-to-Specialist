---
title: "Fig. 2: Plant diversity drives beta diversity changes in the bacterial and fungal communities."
...
This script analyses the soil microbial communities beta diversity via redundancy analysis (panel A), using PERMANOVA to assess which environmental parameters significantly affect soil microbial community structure (panel B).
...
Ordination can be calculated via the `phyloseq` or `vegan` packages. We utilized both to check if results were the same (they were), but ultimately stuck with using `vegan`, as we also used this package for subsequent PERMANOVA. For the RDA, we utilized the objects `ps.bac.norm` and `ps.fun.norm` which are the TSS-transformed and CLR-normalized ASV reads, as stated also in our article's methods section.
...
Load packages, recall required `phyloseq` objects *(see script 'pre-processing')*, and extract data:
...
# extract data 
otu.bac = ps.bac.norm %>% phyloseq::otu_table() %>% t() %>% as.data.frame()
otu.fun = ps.fun.norm %>% phyloseq::otu_table() %>% t() %>% as.data.frame()
data.bac = data.frame(phyloseq::sample_data(ps.bac.norm))
data.fun = data.frame(phyloseq::sample_data(ps.fun.norm))
```

**1.3 Create RDA models with `vegan`**
...
# mixed models
rda_model.bac = vegan::rda(otu.bac ~ plot + psr + TOC + TN + pH, data = data.bac)
rda_model.fun = vegan::rda(otu.fun ~ plot + psr + C + N + pH, data = data.fun)
...
**1.4 Plot the RDA using `vegan` and `ggplot2`**
...
```{r rda plot, tidy=FALSE, message=FALSE, warning=FALSE}
library(ggplot2) # plotting
library(viridis) # color palette 
#?vegan::scores

data.bac$psr = as.factor(data.bac$psr)
data.fun$psr = as.factor(data.fun$psr)

# extract ordination scores 
scores.bac <- scores(rda_model.bac, display = "sites")
scores.fun <- scores(rda_model.fun, display = "sites")

ordination.bac <- data.frame(scores.bac, PSR = data.bac$psr, block = data.bac$plot)
ordination.fun <- data.frame(scores.fun, PSR = data.fun$psr, block = data.fun$plot)
...
Afterwards, we created an illustration for Figure 2 panel B in Inkscape, visually summarizing the output of our PERMANOVA tests.
